...
Native Swift + SwiftUI via SwiftPM (ADR 0007). `models/core.als` is the single source
of truth; `src/Core/` is generated by `oxidtr` and must not be edited by hand.

```
models/          # Alloy source of truth (models/core.als)
src/
  Core/          # GENERATED by oxidtr — do not edit by hand
                 #   Models.swift, Operations.swift, Fixtures.swift
                 #   Tests.swift (excluded from build via Package.swift)
                 #   Support.swift — hand-written
...
Requires **Swift 6 / Xcode 16+**. No other mandatory installs.

```bash
# Install the pre-push hook (SwiftFormat + oxidtr consistency gate):
...
swift run mbt --dir ja-en "こんにちは"

# Regenerate src/Core/ from the Alloy model (after editing models/core.als):
oxidtr generate models/core.als --target swift --output src/Core

# Consistency gate (also wired into git-hooks/pre-push):
